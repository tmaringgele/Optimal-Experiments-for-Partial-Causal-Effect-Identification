...
graph <- igraph::graph_from_literal(X -+ Y,
Ur -+ X, Ur -+ Y)
V(graph)$leftside <- c(0, 0, 0)
V(graph)$latent <- c(0, 0, 1)
V(graph)$nvals <- c(2, 2, 2)
E(graph)$rlconnect <- c(0, 0, 0)
E(graph)$edge.monotone <- c(0, 0, 0)
